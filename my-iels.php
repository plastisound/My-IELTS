<?php
/**
 * My IELS v1.0
 *
 * Custom form and taxonomies for IELS Forms by Merca3w.com
 *
 * @link              http://skattagency.com
 * @since             1.0
 * @package           My_Iels
 *
 * @wordpress-plugin
 * Plugin Name:       My IELS
 * Plugin URI:        http://skattagency.com
 * Description:       Custom form and taxonomies for IELS Forms.
 * Version:           1.0.0
 * Author:            Gerardo Correa B.
 * Author URI:        http://pisd.com.mx
 * License:           GPL-2.0+
 * License URI:       http://www.gnu.org/licenses/gpl-2.0.txt
 */

// If this file is called directly, abort.
if ( ! defined( 'WPINC' ) ) {
	die;
}


if( !class_exists('iels_contact_form') ):

class iels_contact_form
{

	
	/*
	*  Constructor
	*
	*  This function will construct all the neccessary actions, filters and functions.
	*
	*  @type	function
	*  @date	06/12/17
	*  @since	1.0.0
	*
	*  @param	N/A
	*  @return	N/A
	*/
	
	function __construct()
	{
			
		// vars
		$this->settings = array(
			'version'			=> '1.0',
			'upgrade_version'	=> '0.1',
			'include_3rd_party'	=> false
		);
		
		// actions
		add_action('init', array($this, 'init'), 1);/**/
		
		$this->include_before_theme();		
		
	}

	function fix($phpmailer) {
	  	$phpmailer->Sender = $phpmailer->From;
	}
    
        
	/*
	*  include_before_theme
	*
	*  This function will include core files before the theme's functions.php file has been excecuted.
	*  
	*  @type	action (plugins_loaded)
	*  @date	3/09/13
	*  @since	4.3.0
	*
	*  @param	N/A
	*  @return	N/A
	*/
	
	function include_before_theme()
	{	

		// incudes
		include_once('iels-form/form-iels.php');

	}


	/*
	*  init
	*
	*  This function is called during the 'init' action and will do things such as:
	*  create post_type, register scripts, add actions / filters
	*
	*  @type	action (init)
	*  @date	23/06/12
	*  @since	1.0.0
	*
	*  @param	N/A
	*  @return	N/A
	*/
	
	function init()
	{
		add_action('phpmailer_init', array( $this, 'fix' ) );
		add_action( 'init', 'init_states_iels_taxonomy' );
		add_shortcode('IELS_FORM', 'Iels_form_shortcode');
		add_action('wp_enqueue_scripts', 'Iels_Styles');
		
		// admin only
		if( is_admin() )
		{
			//Setup Admin Page
			add_action('admin_menu', 'users_iels_list_menu');
			add_action( 'parent_file', 'menu_highlight' );
		}
	}


}
function jal_install_iels_users() {
		global $wpdb;
		global $jal_db_version;

		$table_name = $wpdb->prefix . 'iels_users';
		
		$charset_collate = $wpdb->get_charset_collate();
 		
		$sql = "
			DROP TABLE IF EXISTS $table_name;
			CREATE TABLE $table_name (
			id mediumint(9) NOT NULL AUTO_INCREMENT,
			nombre text NOT NULL,
			apellidos text NOT NULL,
			email text NOT NULL,
			telefono text NOT NULL,
			ciudad text NOT NULL,
			curso text NOT NULL,
      status text NOT NULL,
			asesor text NOT NULL,
			time date DEFAULT '0000-00-00' NOT NULL,
			UNIQUE KEY id (id)
		) $charset_collate;";

		require_once( ABSPATH . 'wp-admin/includes/upgrade.php' );
		dbDelta( $sql );
		add_option( 'jal_db_version', $jal_db_version );
		
	}
register_activation_hook( __FILE__, 'jal_install_iels_users' );

function iels_contact_form()
{
	global $iels_contact_form;
	
	if( !isset($iels_contact_form) )
	{
		$iels_contact_form = new iels_contact_form();
	}
	
	return $iels_contact_form;
}


// initialize
iels_contact_form();


endif; // class_exists check

?>
